...
struct MealTimeEditView: View {
    ...
    private func save() {
        ...
        case .edit(let setting):
            let timeChanged = setting.hour != hour || setting.minute != minute
            ...
            if timeChanged {
                // Delete future doses for this key so refreshAllDoses regenerates
                // them with the updated scheduled time.
                let mealKey = setting.key
                let startOfToday = Calendar.current.startOfDay(for: Date())
                if let allDoses = try? modelContext.fetch(FetchDescriptor<MedicationDose>()) {
                    for dose in allDoses
                    where dose.mealTimeRaw == mealKey
        ...
        // Regenerates any missing doses (and saves) after the setting change.
